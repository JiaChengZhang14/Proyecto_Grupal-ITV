...
class PropietarioRepositoryImpl(
    ...
    /**
     * función que añade o actualiza la base de datos
     * @author JiaCheng Zhang, Kevin Matute
     * @param entity Es el propietario que se quiere añadir o actualiza en la base de datos
     * @return devuelve el propietario que se ha añadido o actualizado
     */
    override fun save(entity: Propietario): Propietario {
        return getById(entity.dni) ?. let {
            updatePropietario(entity)
        } ?: run {
            insertPropietario(entity)
        }
    }
     ...
     */
    private fun insertPropietario(entity: Propietario): Propietario {
     ...
     * @author JiaCheng Zhang, Kevin Matute
     * @param entity Es el propietario que se quiere actualizar en la base de datos
     * @return devuelve el propietario que se ha actualizado
     */
    private fun updatePropietario(entity: Propietario): Propietario {
        logger.debug { "adding into database" }
        val sql = """UPDATE propietario SET nombre = ?, apellidos = ?, telefono = ?, email = ? WHERE dni = ?;""".trimIndent()

        database.connection.use {
            it.prepareStatement(sql).use { stm ->
                stm.setString(1, entity.nombre)
                stm.setString(2, entity.apellidos)
                stm.setString(3, entity.telefono)
                stm.setString(4, entity.email)
                stm.setString(5, entity.dni)
